...
#	- 원자성 : 트랙잭션 작업은 전부 성공하거나 전부 실패하여 취소 되어야 함
#	- 일관성 : 트랙잭션이 성공적으로 실행되면, 언제나 일관성 있는 상태로 유지 되어야 함
#	- 고립성 : 트랙잭션이 실행되는 동안에 다를 트랙잭션이 현재 작업에 껴들수 없음
#   - 영속성 : 성공적으로 완료된 트랙잭션의 결과는 시스템 장애가 발생해도 영구적으로 반영되어야 함

# MYSQL은 자동 커밋이 설정 되어 있어서 우리가 작성한 쿼이들이 바로 커빗됨 

# START TRANSACTION/BEGIN/BEGIN WORK : 트랙잭션 시작
# ROLLBACK : 트랙잭션 시작 전상태로 되돌림
