...
public class AdminController {
	...
	@GetMapping("/board/list")//실제 URL : /admin/board/list   (위에있는@RequestMapping가 /admin추가했습)
	...
	public String postBoardList(@RequestParam("board")String name) {
		// 서비스에게 name을 주면서 게시판을 등록하라고 요청
		postService.insertBoard(name);
		return "redirect:/admin/board/list";// (위에있는@RequestMapping은 return에 쓸수없습)
	}
	...
	public String boardUpdate( 
			//화면에서 보낸 게시판명을 가져옴
			@RequestParam("board2")String name,
			@PathVariable("num")int num) {
		//게시판번호, 이름을 이용하여 서비스에게 게시판명을 수정하라고 요청
		postService.updateBoard(name,num);
		return "redirect:/admin/board/list";
	}
