...
public class Student implements Serializable{
	...
	public boolean addSubjectScore(SubjectScore subjectScore) {
		// 등록된 성적인니 확인하기 위해 성적 목록에서 몇번지에 있는지 확인
		int index = list.indexOf(subjectScore);
		//없으면(-1번지이면) 성적을 추가 => true를 반화(추가했으니까)
		if(index < 0) {
		...
		//있으면 기존 성적에 새 점수로 수정 => false를 반화(추가가 아닌 수정했으니까)
		//기존 성적 정보
	...
	public boolean removeSubjectScore(Subject subject) {
		...
		//remove는 objects,equals를 호츌 => 두 객체의 클래스가 다르면 무조건 false
