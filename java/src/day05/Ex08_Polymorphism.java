...
public class Ex08_Polymorphism {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 매개변수의 다형성
		 * - 매서드의 매개 변수로 자식 클래스들이 들어가야 하고, 메서드 안 내용이 같은 경우
		 * 	 부모 클래스를 매개변수로 지정하는 것
		 * 객체의 다형성
		 * - 부모 클래스의 객체를 이용하여 다양한 자식 클래스의 객체를 관리할수 있음
		 * 
		 * intance of
	...
	public static void printInstanceOf(Object obj) {
		if(obj instanceof Circle) {
			System.out.println("객체는 Circle로 변환이 가능");
		}else if(obj instanceof Rect) {
			System.out.println("객체는 Rect로 변환이 가능");
		}else if(obj instanceof Shape) {
			System.out.println("객체는 Shape로 변환이 가능");
		}else {
			System.out.println("객체는 도형으로 변환이 불가능");
		}
	}
