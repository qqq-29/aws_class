...
public class Ex09_Stream {
	...
	public static void main(String[] args) {
		/* 스트림 p493
		 * - 왜 사용?
		 * 	 - 배열이나 컬렉션 인터페이스의 정보들을 일관된 방법으로 처리할 수있어서
		 * 	 - 중간 처리메서드와 최종 처리 메서드를 제공해서 여러 연산을 한번에 할수 있음
		 * - filter
		 * 	 - 중간 처리 메서드
		 * 	 - 객체들 중 원하는 객체들만 고를 때 사용
		 * - map
		 * 	 - 중간 처리 메서드
		 * 	 - 객체를 이용하여 다른 형테의 결과를 얻을 떄 사용
		...
		list.add(new Student(2,1,1,"홍동"));
		list.add(new Student(3,1,1,"홍길"));
		
		//List를 stream으로 변환
		Stream<Student> stream = list.stream();
		//1학년 1반 1번 학생을 출력
		stream.filter(s->s.getGrade() == 1)
			 .filter(s->s.getClassNum() == 1)
			 .filter(s->s.getNum() == 1)
			 .forEach(s->System.out.println(s));
		//등록된 학생 수
		stream = list.stream();
		System.out.println("등록된 학생 수 : " + 
